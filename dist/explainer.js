...
var _parse_travel_period = /*#__PURE__*/_classPrivateFieldLooseKey("parse_travel_period");

var _parse_date = /*#__PURE__*/_classPrivateFieldLooseKey("parse_date");

var _date_to_text = /*#__PURE__*/_classPrivateFieldLooseKey("date_to_text");

class Explainer {
  constructor(text) {
    Object.defineProperty(this, _date_to_text, {
      value: _date_to_text2
    });
    Object.defineProperty(this, _parse_date, {
      value: _parse_date2
    });
    Object.defineProperty(this, _parse_travel_period, {
  ...
  listForm() {
    let list = '<ul>';
    this.issued_until != null ? list += '<li>Issued Until: ' + _classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.issued_until, 'de') + '</li>' : {};
    this.min_stay != null ? list += '<li>Min. Stay: ' + this.min_stay + ' days' : {};
    ...
    this.cancelable === 'yes' || this.cancelable === 'no' ? list += '<li>Cancelable: ' + this.cancelable + '</li>' : list += '<li>Cancelable: ' + this.cancelable.currency + this.cancelable.price + '</li>';
    this.change === 'yes' || this.change === 'no' ? list += '<li>Change: ' + this.change + '</li>' : list += '<li>Rebooking: ' + this.change.currency + this.change.price + '</li>';
    ...
    this.advanced_reservation_days != null ? list += '<li>Advanced Reservation : ' + this.advanced_reservation_days + ' days</li>' : {};
    ...
    if (this.travel_period != null) {
      ...
      for (var i = 0; i < this.travel_period.length; i++) {
        list += '<li>' + _classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.travel_period[i].from, 'de') + ' - ' + _classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.travel_period[i].to, 'de') + '</li>';
      }
    ...
    if (this.travel_commenced != null) {
      ...
      for (var i = 0; i < this.travel_commenced.length; i++) {
        list += '<li>' + _classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.travel_commenced[i].from, 'de') + ' - ' + _classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.travel_commenced[i].to, 'de') + '</li>';
      }
    ...
    if (this.travel_period_blackout != null) {
      ...
      for (var i = 0; i < this.travel_period_blackout.length; i++) {
        list += '<li>' + _classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.travel_period_blackout[i].from, 'de') + ' - ' + _classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.travel_period_blackout[i].to, 'de') + '</li>';
      }
    ...
    if (this.travel_period_from != null) {
      ...
      for (var i = 0; i < this.travel_period_from.length; i++) {
        list += '<li>' + _classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.travel_period_from[i].from, 'de') + ' - ' + _classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.travel_period_from[i].to, 'de') + '</li>';
      }
    ...
    if (this.travel_period_to != null) {
      ...
      for (var i = 0; i < this.travel_period_to.length; i++) {
        list += '<li>' + _classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.travel_period_to[i].from, 'de') + ' - ' + _classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.travel_period_to[i].to, 'de') + '</li>';
      }

      list += '</ul></li>';
    }

    this.child_charge != null ? list += '<li>Child Charge: ' + this.child_charge * 100 + '%</li>' : {};
    this.infant_charge != null ? list += '<li>Infant Charge: ' + this.infant_charge * 100 + '%</li>' : {};
    this.infant_seat_charge != null ? list += '<li>Infant with Seat Charge: ' + this.infant_seat_charge * 100 + '%</li>' : {};
    return list + '</ul>';
  }
  ...
  get travel_commenced() {
    ...
    return {
      from: found[1] ? _classPrivateFieldLooseBase(this, _parse_date)[_parse_date](found[1]) : null,
      to: found[2] ? _classPrivateFieldLooseBase(this, _parse_date)[_parse_date](found[2]) : null
    };
  }

  get travel_period_blackout() {
    return _classPrivateFieldLooseBase(this, _parse_travel_period)[_parse_travel_period](/TRAVEL IS NOT PERMITTED ([A-Z0-9 ]+)./);
  }
  ...
  get sunday_rule() {
    ...
    return true; //let findand = this.text.match(/AND - TRAVEL FROM/);
  ...
  get child_charge() {
    let found = this.text.match(/ACCOMPANIED CHILD 2-11(?:\. ID REQUIRED|) - (?:CHARGE |)(\d{1,3}|NO DISCOUNT) (?:PERCENT OF THE FARE\.|OR )/);
  ...
  get infant_charge() {
    let found = this.text.match(/INFANT UNDER 2 WITHOUT A SEAT - CHARGE (\d{1,3}|NO DISCOUNT) PERCENT OF THE FARE\./);
  ...
  get infant_seat_charge() {
    let found = this.text.match(/INFANT UNDER 2 WITH A SEAT(?:\. ID REQUIRED|) - (?:CHARGE |)(\d{1,3}|NO DISCOUNT) (?:PERCENT OF THE FARE\.|OR )/);
  ...
  explain(lang = 'en') {
    ...
    if ('de' === lang) {
      result += `Buchbar ist der Tarif ${this.issued_until ? `bis zum ${_classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.issued_until, lang)} ` : ''}für `;
      ...
      if (this.travel_period) {
        ...
        if (this.travel_commenced.from && this.travel_commenced.to) {
          ...
          result += `Abflüge nach dem ${_classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.travel_commenced.from, lang)}`;
        } else if (this.travel_commenced.to) {
          result += `Abflüge bis spätestens ${_classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.travel_commenced.to, lang)}`;
        }
      } else {
      ...
      if (this.min_stay) {
        result += ` Der Mindestaufenthalt beträgt ${this.min_stay} Tage${true === this.sunday_rule ? ` oder eine Nacht von Samstag auf Sonntag` : ''}. `;
      }

      if (this.max_stay) {
        result += `Der maximale Aufenthalt beträgt ${1 === this.max_stay ? `einen Monat` : `${this.max_stay} Monate`}. `;
      }
      ...
      // Fallback to en
      result += `This fare can be booked ${this.issued_until ? `until ${_classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.issued_until, lang)} ` : ''}for `;

      if (this.travel_period) {
        result += `trips between ${this.travel_period.map(({
          from,
          to
        }) => this.constructor.month_day_period_to_yearly_periods(from, to, this.now)).flat().sort((a, b) => a.from > b.from).map(period => `${_classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](period.from, lang)} ${period.from == null ? '' : ' - '} ${_classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](period.to, lang)}`).join(', ')}`;
      } else if (this.travel_period_from) {
        result += `departures between ${this.travel_period_from.map(({
          from,
          to
        }) => this.constructor.month_day_period_to_yearly_periods(from, to, this.now)).flat().sort((a, b) => a.from > b.from).map(period => `${_classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](period.from, lang)} - ${_classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](period.to, lang)}`).join(', ')} `;

        if (this.travel_period_to) {
          result += `and inbound flights between ${this.travel_period_to.map(({
            from,
            to
          }) => this.constructor.month_day_period_to_yearly_periods(from, to, this.now)).flat().sort((a, b) => a.from > b.from).map(period => `${_classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](period.from, lang)} - ${_classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](period.to, lang)}`).join(', ')} `;
        }
      } else if (this.travel_period_to) {
        result += `inbound flights between ${this.travel_period_to.map(({
          from,
          to
        }) => this.constructor.month_day_period_to_yearly_periods(from, to, this.now)).flat().sort((a, b) => a.from > b.from).map(period => `${_classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](period.from, lang)} - ${_classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](period.to, lang)}`).join(', ')} `;
      } else if (this.travel_commenced) {
        if (this.travel_commenced.from && this.travel_commenced.to) {
          result += `Departures between ${_classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.travel_commenced.from, lang)} und ${_classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.travel_commenced.to, lang)}`;
        } else if (this.travel_commenced.from) {
          result += `Departures after ${_classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.travel_commenced.from, lang)}`;
        } else if (this.travel_commenced.to) {
          result += `Departures before ${_classPrivateFieldLooseBase(this, _date_to_text)[_date_to_text](this.travel_commenced.to, lang)}`;
        }
      } else {
      ...
      if (this.min_stay) {
        result += ` The minimum stay requirement is of ${this.min_stay} days${true === this.sunday_rule ? ` (or a Sunday)` : ''}. `;
      }

      if (this.max_stay) {
        result += `Furthermore, your stay cannot exceed ${1 === this.max_stay ? `one month` : `${this.max_stay} months`}. `;
      }
  ...
  static month_day_period_to_yearly_periods(from, to, now = new Date()) {
    ...
    if (new Date(this_year_from + '-' + from) > new Date(this_year_to + '-' + to)) {
      this_year_to++;
      next_year_to++;
    }

    if (now < new Date(this_year_from + '-' + from) && now < new Date(this_year_to + '-' + to)) {
      // Period is in the future
      return [{
        from: new Date(this_year_from + '-' + from).toISOString().substring(0, 10),
        to: new Date(this_year_to + '-' + to).toISOString().substring(0, 10)
      }];
    } else if (now > new Date(this_year_from + '-' + from) && now > new Date(this_year_to + '-' + to)) {
      // Period is in the past
      return [{
        from: new Date(next_year_from + '-' + from).toISOString().substring(0, 10),
        to: new Date(next_year_to + '-' + to).toISOString().substring(0, 10)
      }];
    } else if ( // Currently in Period
    now > new Date(this_year_from + '-' + from) && now < new Date(this_year_to + '-' + to)) {
      return [{
        from: new Date(this_year_from + '-' + from).toISOString().substring(0, 10),
        to: new Date(this_year_to + '-' + to).toISOString().substring(0, 10)
      }, {
...
function _parse_travel_period2(regex) {
  ...
  let periods = found[1].split('OR').map(period => {
    let [from, to] = period.split('THROUGH').map(date => _classPrivateFieldLooseBase(this, _parse_date)[_parse_date](date));
    return {
      from,
      to
    };
...
function _parse_date2(date) {
  let [, day, month_name, year = null] = date.trim().match(/(\d{2})(\w{3})(?: (\d{2})|)/);
  return (year ? '20' + year + '-' : '') + months[month_name] + '-' + day;
}

function _date_to_text2(date, lang = 'en') {
  if (date == null) {
    return 'bis zum ';
  }

  if (5 === date.length) {
    return new Date(new Date().getFullYear() + '-' + date).toLocaleDateString(lang, {
      day: 'numeric',
      month: 'long'
    });
  }
